...
import ShaderToyCanvas from "./ShaderToyCanvas";
import { softGradient, linearGradient, parallaxStars, blobGradient, auroraWave } from "@/shaders/shaders";
import ShaderControls from "./ShaderControls";

const defaultColorSets = [
  { color1: "#aaa7db", color2: "#ffffff", color3: "#ffffff", color4: "#ffffff", color5: "#ffffff" },
  { color1: "#a7dbce", color2: "#d0ebff", color3: "#d7f5fc", color4: "#ffffff", color5: "#ffffff" },
  { color1: "#a7dbce", color2: "#d0ebff", color3: "#fcd6d6", color4: "#ffffff", color5: "#ffffff" },
  { color1: "#dbbea7", color2: "#fffed1", color3: "#f3ffd3", color4: "#ffffff", color5: "#ffffff" },
];
...
const MeshGradientwhatamesh: React.FC<{
  ...
  const [speedValue, setSpeedValue] = useState<number>(speed ?? 1.3);

  // Spread and rotation uniform values
  const [spreadValue, setSpreadValue] = useState<number>(spread ?? 1.0);
  const [rotationValue, setRotationValue] = useState<number>(rotation ?? Math.PI / 4);
  ...
  // return <ShaderCanvas plugin={gradientShaderPlugin} initialColors={initialColors} />;
  return (
    <div>
      {/* <img
      ...
      */}
      ...
      {(() => {
        // Extract the first non-empty segment of the pathname (after the leading "/")
        const pathnameSegment = typeof window !== "undefined"
          ? window.location.pathname.split("/").filter(Boolean)[0] ?? "0"
          : "0";

        const index = Number(pathnameSegment);

        // Fetch shader config by index (fallback to first entry)
        const selectedConfig = !Number.isNaN(index) && shaderConfigs[index]
          ? shaderConfigs[index]
          : shaderConfigs[0];

        const selectedShader = selectedConfig.shader;
      ...
      {showControls && (
        <div className="fixed bottom-5 left-5 flex items-center gap-2 z-20">
          <button
            onClick={() => {
              const pathnameSegment = window.location.pathname.split("/").filter(Boolean)[0] ?? "0";
              const index = Number(pathnameSegment);
              const prev = (index - 1 + shaderConfigs.length) % shaderConfigs.length;
